function PostComment(name, email, rawcontent, rep) {
    ...
    let modal = mkAutoAlertModal('请稍后...', '正在向服务器递交您的评论', () => {
        ajax_post('/api/commits/post.php', { a: g_artID, n: name, e: emailhash, c: content, r: rep }, w = {
            success: (data) => {
...
function ReloadComment(page) {
    ajax_get('/api/commits/get.php', { a: g_artID, p: page }, w = {
        before: () => {
            $("#comments").empty();
        },
        success: (data) => {
            ...
            if (d['status'] == 'ok') {
                ...
                d['data']['commits'].forEach((x) => {
                    ...
                    commitHead.append('<div><img src="/api/commits/getavator.php?hash=' + x['email'] + '"></div>');
