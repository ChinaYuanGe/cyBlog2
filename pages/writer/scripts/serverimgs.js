function GetServerImages(){
    ...
    ajax_post('/api/arts/getimagelist.php',{id:artID},{
        before:()=>{
            holder.empty();
        ...
        success:(data)=>{
            ...
            holder.empty();
            if(jData['status'] == 'ok'){
                var imgs = jData['data']['imgs'];
                for(var i=0;i<imgs.length;i++){
                    let thisImg = imgs[i];
                    let picHolder = $("<div class=\"mb-3\">");
                    picHolder.append('<img class="prevServerPic mb-2" src="/img/arts/'+imgs[i]+'">');
                    ...
                    insertBtn.click(function(){
                        tinymce.activeEditor.insertContent('<img src="/img/arts/'+thisImg+'">');
                        $('#serverPicList').modal('hide');
                    });
                    deleteBtn.click(function(){
                        var modal = mkAutoAlertModal('请求删除','像服务器请求删除选中图片...',()=>{
                            ajax_post('/api/arts/deleteimage.php',{id:artID,pic:thisImg},{
                                success:(data)=>{
                                    GetServerImages();
                                },
                                fail:(t)=>{
                ...
                mkAlertModal('获取失败','信息获取失败:'+jData['data']['msg']);
            }
            else{
                mkAlertModal('获取失败','服务器发送了无效的响应');
            }
        },
        fail:(t)=>{
            holder.empty();
